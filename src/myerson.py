...
class Bidder:
    ...
    def virtual_value(self, x):
        return self.formula_bid(x) - self.shading_factor[0] * (1 - self.distrib.cdf(x)) / (
                               self.distrib.pdf(x))
    def reversed_virtual_value(self,x):
        return (x+1)/2
...
class Seller:
    ...
    def run_auction(self, list_bids):
        ...
        elif self.type_auction == "myerson":
             #print(list_bids)
             print(list_bids)
             bid_virtualized = [bidder.virtual_value(bid) for bidder,bid in zip(self.list_bidders,list_bids)]
             print(bid_virtualized)
             winner = np.random.choice((np.flatnonzero(bid_virtualized == max(bid_virtualized))))
             if bid_virtualized[winner] >= 0:
                 winning_bidder = self.list_bidders[winner]
                 payment = np.sort(bid_virtualized)[-2]
                 print(payment)
                 #print(f"snd virt bid:{payment}")
                 if payment > 0:
                    payment = winning_bidder.reversed_virtual_value(payment)
                 else:
                    payment = self.reserve_price[winner]
                 #print(f"true payment:{payment}")
                 self.list_bidders[winner].update_revenue(payment)
                 self.revenue += payment
             else:
                 print("error")
